...
<div class="w-full lg:w-2/4 bg-white shadow-md flex flex-col items-center px-3 mx-auto pb-5">
    <form action="/blog/{{$post->slug}}" method="POST" enctype="multipart/form-data" >  @csrf @method('PUT')
      ...
      <div class="mb-4">
        ...
        <input class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('title') border-red-500 @enderror" type="text" placeholder="Enter title" name="title" value="{{$post->title}}">
        @error('title')
      ...
      <div class="mb-4">
        ...
        <textarea class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('description') border-red-500 @enderror" cols="30" rows="10" id="equipe" type="text" placeholder="Enter a description" name="description">{{$post->description}}</textarea>
        @error('description')
          <p class="text-red-500 text-xs italic">{{ $message }}</p>
        @enderror
      </div>
      ...
      <div class="mb-4">
        ...
        <select class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline value="{{$post->category}}" @error('category') border-red-500 @enderror" id="category" name="category">
