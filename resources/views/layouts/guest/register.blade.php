...
<body class="flex flex-col items-center justify-center w-screen h-screen bg-gray-200 text-gray-700">
	...
	<form class="flex flex-col bg-white rounded shadow-lg p-12 mt-12" method="POST" action="{{ route('register') }}">
        @csrf <!-- Add CSRF token for Laravel form submission -->
    
        <label class="font-semibold text-xs" for="usernameField">Username</label>
        <input name="name" class="flex items-center h-12 px-4 w-64 bg-gray-200 mt-2 rounded focus:outline-none focus:ring-2" type="text">
        ...
        <label class="font-semibold text-xs mt-3" for="passwordConfirmationField">Confirm Password</label>
