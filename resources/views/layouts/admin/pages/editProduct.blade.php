    ...
    <div class="p-2 max-w-md mx-auto my-10 bg-white p-5 rounded-lg shadow-lg">
        ...
        <form method="POST" action="{{ route('products.update', $product->id) }}" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="product_name" id="product_name" class="border rounded-lg px-3 py-2 w-full focus:outline-none focus:ring focus:border-blue-300" value="{{ $product->product_name }}" required>
                @error('product_name')
            ...
            <div class="mb-4">
                ...
                <textarea name="product_description" id="product_description" class="border rounded-lg px-3 py-2 w-full focus:outline-none focus:ring focus:border-blue-300" rows="4" required>{{ $product->product_description }}</textarea>
            ...
            <div class="mb-4">
                ...
                <input type="number" name="price" id="price" class="border rounded-lg px-3 py-2 w-full focus:outline-none focus:ring focus:border-blue-300" step="0.01" value="{{ $product->price }}" required>
                @error('price')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            <div class="mb-4">
                ...
                <input type="number" name="quantity" id="quantity" class="border rounded-lg px-3 py-2 w-full focus:outline-none focus:ring focus:border-blue-300" value="{{ $product->quantity }}" required>
                @error('quantity')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mt-8">
                <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 transition duration-200">Update Product</button>
            </div>
        </form>
    </div>
</div>
